...
right order, provides the glue between them \cgg{I would remove the ``provides the glue between them'' clause, as it adds little meaning to the sentence and seems redundant.} and allows contract proof to run
...
contracts (the white rectangle in the center of the figure \cgg{There is no longer a white rectange with the propositional properties. Where did this picture go? Should we remove this clause or update the picture?}) using the toolbox (right-hand side of Figure~\ref{fig:eclipse_frontend}).
...
DSLTrans model transformation under analysis and of the contracts to be proved \cgg{The EGL does not parse the Ecore models. Parsing is converting the concrete syntax into an abstract syntax. The Ecore models are parsed by the EMF runtime, which converts the text (xml) into a memory representation. The EGL is more like a visitor of the abstract syntax tree of the Ecore models. I would remove this sentence and start the next one as ``the EGL transformations produce...''}.
...
Section~\ref{sec:gen_models_mt}\cgg{Unumbered sections cannot be refered to like this. Either we use ``in the previous subsection'' or we use ``2'' to refer to that section.}, need to be generated for the property proof
algorithm to execute\cgg{In the previous section, only one model transformation was described as being generated: the dsltrans. The contract, at least in the way it is described, does not correspond to a model transformation.}. These are the \emph{path condition generation} T-Core
...
Figure~\ref{fig:arch}), following the ordering of the rules in the DSLTrans transformation being analysed \cgg{I would add ``(artifact (a) in
Figure~\ref{fig:arch})'' here.}).
...
pre-condition's model, the appropriate property T-Core matcher \cgg{I would add ``from the artifact marked (b)''} will try to find
...
consist of a set of queries to path condition models \cgg{This last sentence seems redundant, provided you add an explicit reference to the artifact marked (b) above.}.
